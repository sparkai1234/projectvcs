#!/usr/bin/env python3
"""
AI-Powered Korean VC Intelligence - People & Company Search
한국 벤처캐피털 AI 정보 시스템 - 인물 및 회사 검색
"""
...
class AIVCIntelligencePeople:
    def __init__(self):
        self.data_loaded = False
        self.companies_summary = ""
        self.people_summary = ""
        print("🤖 한국 벤처캐피털 AI 정보 시스템을 초기화중... (인물 + 회사 검색)")
    ...
    def fetch_full_data(self):
        ...
        # Load contacts (people) - Use Contacts Matched for efficiency
        try:
            url = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Contacts%20Matched"
            params = {'maxRecords': 500}  # Load more contacts for people search
            response = requests.get(url, headers=headers, params=params)
            
            if response.status_code == 200:
                contact_data = response.json().get('records', [])
                print(f"   ✅ {len(contact_data)}개 매칭된 연락처 로딩 완료")
            else:
                contact_data = []
                print(f"   ❌ 매칭된 연락처 데이터 로딩 실패: {response.status_code}")
        except Exception as e:
            contact_data = []
            print(f"   ❌ 매칭된 연락처 데이터 로딩 오류: {e}")
        
        # Store data
        self.vc_companies = vc_data
        self.funds = fund_data
        self.contacts = contact_data  # Now using Contacts Matched for people search
        ...
        print("✅ 종합 데이터 로딩 성공!")
        print(f"📊 요약: {len(vc_data)}개 VC, {len(fund_data)}개 펀드, {len(contact_data)}개 매칭된 연락처")
    ...
    def create_summaries(self):
        ...
        # People names
        people_names = []
        for record in self.contacts[:50]:  # Sample first 50 people
        ...
        self.people_summary = f"사용 가능한 인물들: {', '.join(people_names[:20])}"
...
- "Tell me about Kakao Ventures" → "COMPANY:카카오벤처스"
...
응답:"""
    ...
    def display_person_info(self, person_data, person_name):
        """인물 정보 표시"""
        print(f"✅ 인물 정보 발견! (매칭 점수: {person_data['match_score']}%)")
        print()
        ...
        if person.get('직책') or person.get('Position'):
            position = person.get('직책', '') or person.get('Position', '')
            print(f"💼 직책: {position}")
        if person.get('전화번호') or person.get('Phone'):
            phone = person.get('전화번호', '') or person.get('Phone', '')
            print(f"📞 연락처: {phone}")
        if person.get('이메일') or person.get('Email'):
            email = person.get('이메일', '') or person.get('Email', '')
            print(f"📧 이메일: {email}")
        ...
        if company_name:
            ...
            if related_vc and best_score >= 70:
                ...
                print(f"'{company_name}' 회사 정보를 찾을 수 없습니다.")
    ...
    def query(self, user_query):
        ...
        if query_type == "PERSON":
            ...
            if person_data:
                ...
                print(f"❌ '{query_name}' 인물 정보를 찾을 수 없습니다.")
                print("💡 사용 가능한 인물들:")
...
def main():
    """한국어 우선 대화형 AI 인터페이스 - 인물 + 회사 검색"""
    print("🤖 한국 벤처캐피털 AI 정보 시스템 (인물 + 회사)")
    print("=" * 60)
    
    # Initialize system
    intel = AIVCIntelligencePeople()
    intel.fetch_full_data()
    
    print("\n💡 예시 질문:")
    print("   👤 인물: 황병선, 강승순, 김철수")
    print("   🏢 회사: 빅뱅벤처스, 미래에셋벤처투자") 
    print("   💬 자연어: '황병선이 누구인가요?', '빅뱅벤처스 정보'")
    
    while True:
        print("\n" + "="*50)
        query = input("\n🎤 인물이나 회사에 대해 궁금한 것을 물어보세요 ('종료'): ").strip()
