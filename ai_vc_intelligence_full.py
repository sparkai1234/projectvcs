#!/usr/bin/env python3
"""
AI-Powered Korean VC Company Intelligence - Full Data Version
Loads more data for comprehensive testing
"""
...
class AIVCIntelligenceFull:
    def __init__(self):
        self.data_loaded = False
        self.companies_summary = ""
        print("🤖 Initializing AI-Powered Korean VC Intelligence (Full Data)...")
    ...
    def fetch_full_data(self):
        ...
        try:
            ...
            params = {'maxRecords': 200}  # Load more VCs
        ...
        try:
            ...
            params = {'maxRecords': 500}  # Load more funds
            response = requests.get(url, headers=headers, params=params)
        ...
        try:
            ...
            params = {'maxRecords': 200}  # Load more contacts
            response = requests.get(url, headers=headers, params=params)
        ...
        print(f"📊 Summary: {len(vc_data)} VCs, {len(fund_data)} funds, {len(contact_data)} contacts")
    ...
    def create_companies_summary(self):
        ...
        self.companies_summary = f"Available Korean VC companies: {', '.join(company_names[:30])}"
        if len(company_names) > 30:
            self.companies_summary += f" and {len(company_names) - 30} more companies"
...
Company name:"""
    ...
    def find_company_data(self, company_name):
        """Find company data using fuzzy matching with lower threshold"""
        best_vc_match = None
        best_vc_score = 0
        
        # Search VC companies with more flexible matching
        for record in self.vc_companies:
            name = record.get('fields', {}).get('Company Name', '')
            if name:
        ...
        # Find related funds with flexible matching
        company_funds = []
        for record in self.funds:
            fund_company = record.get('fields', {}).get('Company Name (Korean)', '')
            if fund_company:
                score = fuzz.ratio(company_name, fund_company)
                partial_score = fuzz.partial_ratio(company_name, fund_company)
                final_score = max(score, partial_score)
                
                if final_score >= 70:  # Lower threshold for better matching
                    company_funds.append(record)
        
        # Find related contacts with flexible matching
        company_contacts = []
        for record in self.contacts:
            ...
            if matched_company:
                ...
                partial_score = fuzz.partial_ratio(company_name, matched_company)
                final_score = max(score, partial_score)
                
                if final_score >= 70:  # Lower threshold for better matching
                    company_contacts.append(record)
        
        if best_vc_score >= 50 or company_funds or company_contacts:  # Lower threshold
            return {
                'vc_info': best_vc_match if best_vc_score >= 50 else None,
                'funds': company_funds,
...
"""
    ...
    def query_company(self, user_query):
        ...
        if not self.data_loaded:
            print("❌ Data not loaded. Run fetch_full_data() first.")
            return
        ...
        if not company_data:
            ...
            print("💡 Available companies in loaded data:")
            # Show some available company names
            available_names = []
        ...
        print(f"✅ Found company data! (Match score: {company_data['match_score']}%)")
        print()
...
def main():
    """Interactive AI-powered interface with full data"""
    print("🤖 AI-Powered Korean VC Intelligence System (Full Data)")
    print("=" * 60)
    
    # Initialize system
    intel = AIVCIntelligenceFull()
    intel.fetch_full_data()
    
    print("\n💡 Example queries:")
    print("   • 미래에셋벤처투자")
    print("   • Tell me about Mirae Asset") 
    print("   • What funds does 젠엑시스 operate?")
    print("   • Show me info on companies")
