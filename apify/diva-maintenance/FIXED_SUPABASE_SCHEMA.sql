-- =====================================================
-- 🏦 DIVA FINANCIAL STATEMENTS: SEPARATE TABLES SCHEMA (FIXED)
-- =====================================================
...
-- FIX: Resolved ambiguous column references
...
-- FIXED: Annual data summary by year (resolved ambiguous column reference)
...
FULL OUTER JOIN diva_income_statements is_ 
    ON bs.company_name = is_.company_name 
    AND bs.data_year = is_.data_year
GROUP BY COALESCE(bs.data_year, is_.data_year)
ORDER BY COALESCE(bs.data_year, is_.data_year) DESC;
