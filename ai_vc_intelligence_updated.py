#!/usr/bin/env python3
"""
AI-Powered Korean VC Company Intelligence (Updated)
Combines Airtable data with OpenAI for natural language queries and insights
Updated to work with the new Contacts Matched table structure
"""
...
class AIVCIntelligenceUpdated:
    def __init__(self):
        self.data_loaded = False
        self.companies_summary = ""
        print("🤖 Initializing Updated AI-Powered Korean VC Intelligence...")
    ...
    def fetch_sample_data(self):
        ...
        # Load matched contacts (using new structure)
        try:
            url = f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/Contacts%20Matched"
            params = {'maxRecords': 50}  # Get 50 matched contacts for testing
            response = requests.get(url, headers=headers, params=params)
...
Company name:"""
    ...
    def find_company_data(self, company_name):
        ...
        # Find related contacts (using new structure)
        company_contacts = []
    ...
    def generate_ai_insights(self, company_data, company_name):
        ...
        try:
            ...
            if company_data['contacts']:
                ...
                for contact in company_data['contacts'][:3]:  # Show first 3 contacts
...
"""
    ...
    def query_company(self, user_query):
        ...
        # Display contacts (updated for new structure)
        ...
        if company_data['contacts']:
            ...
            for i, contact in enumerate(company_data['contacts'][:5], 1):  # Show first 5
                contact_fields = contact['fields']
                contact_name = contact_fields.get('Contact Name', 'Unknown')
                ...
                print(f"\n{i:2d}. {contact_name}")
                if contact_position:
                    print(f"    💼 Position: {contact_position}")
                if contact_email:
                    print(f"    📧 Email: {contact_email}")
                if contact_phone:
                    print(f"    📱 Phone: {contact_phone}")
                print(f"    🏢 Original Company: '{original_company}' (Match: {match_score}%)")
        else:
...
def main():
    ...
    print("🤖 AI-Powered Korean VC Intelligence System (Updated)")
    print("=" * 60)
