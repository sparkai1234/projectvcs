// Enhanced script that uses separate fields for better data organization
// This version takes advantage of schema.bases:write permissions
...
(async () => {
    ...
    // Enhanced function to transform Korean data to separate Airtable fields
    const transformDataForAirtable = (item) => {
        return {
            'Company Name': item.operInstNm || '', // Main company name field (matches current Airtable structure)
            'Company ID': item.operInstId || '',
            'Industry': item.comIndNm || '',
            'Notes': `Imported from Korean VC database on ${new Date().toISOString()}`,
            ...
            'PF Angel': item.pfAnglYn === 'Y' ? 'Yes' : 'No',
            'Data Type': dataType === 'VC' ? 'VC Company' : 'Fund Company'
        };
    };

    // Fallback function with minimal fields (in case detailed mapping fails)
    const transformDataForAirtableMinimal = (item) => {
        return {
    ...
    const commonQueryParams = 'sortOrder=&sortDirection=&comIndCdArr=1&_comIndCdArr=on&comIndCdArr=2&_comIndCdArr=on&comIndCdArr=3&_comIndCdArr=on&comIndCdArr=4&_comIndCdArr=on&comIndCdArr=5&_comIndCdArr=on&comIndCdArr=6&_comIndCdArr=on&comIndCdArr=7&_comIndCdArr=on&comIndCdArr=8&_comIndCdArr=on&comIndCdArr=9&_comIndCdArr=on&_invstComLocCdArr=on&_invstComLocCdArr=on&_invstComLocCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_localCdArr=on&_operScaleCdArr=on&_operScaleCdArr=on&_operScaleCdArr=on&_operScaleCdArr=on&_operScaleCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_invstCharcCdArr=on&_comBzcarrCdArr=on&_comBzcarrCdArr=on&_comBzcarrCdArr=on&_comBzcarrCdArr=on&_comBzcarrCdArr=on&_comBzcarrCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&_invstorTpCdArr=on&sc=operInstNm&sv=';
    ...
    if (results.length > 0) {
        ...
        try {
            ...
            let useDetailedMapping = true;
            
            // Function to make HTTP request to Airtable
            const sendToAirtable = (records) => {
                return new Promise((resolve, reject) => {
                    const transformedRecords = records.map(item => ({
                        fields: useDetailedMapping ? transformDataForAirtable(item) : transformDataForAirtableMinimal(item)
                    }));
            ...
            for (let i = 0; i < results.length; i += 10) {
                ...
                try {
                    ...
                    // If detailed mapping fails, try again with minimal mapping
                    if (useDetailedMapping && error.message.includes('422')) {
                        console.log('⚠️  Detailed field mapping failed, falling back to minimal mapping...');
                        useDetailedMapping = false;
                        await sendToAirtable(batch);
                        console.log(`Sent batch ${Math.floor(i / 10) + 1} (minimal mapping)...`);
                    } else {
                        throw error;
                    }
                }
            ...
            console.log(`Used ${useDetailedMapping ? 'detailed' : 'minimal'} field mapping.`);
